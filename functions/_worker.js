...
Return ONLY valid JSON, no other text.`;
                ...
                if (path === '/claude/smart' && method === 'POST') {
                    ...
                    // Get user's accessible projects for security filtering
                    const { results: userProjects } = await env.DB.prepare(
                        'SELECT p.id FROM projects p INNER JOIN project_members m ON p.id = m.project_id WHERE m.user_id = ?'
                    ).bind(user.id).all();
                    const projectIds = userProjects.map(p => p.id);

                    // Define tools that Claude can use to query D1 dynamically
                    const tools = [
                        {
                            name: 'query_tasks',
                            description: 'Query tasks from the database. Only returns tasks the user has access to (assigned to them or in their projects). Use this to answer questions about tasks, find specific tasks, check statuses, priorities, due dates, etc. When user asks about "my tasks" or "my work", set assigned_to_me=true.',
                            input_schema: {
                                type: 'object',
                                properties: {
                                    filters: {
                                        ...
                                        properties: {
                                            assigned_to_me: { type: 'boolean', description: 'If true, only return tasks assigned to the current user. Use this when user asks about "my tasks" or "my work".' },
                                            status: { type: 'string', enum: ['pending', 'completed'], description: 'Filter by task status' },
                                            priority: { type: 'string', enum: ['high', 'medium', 'low', 'none'], description: 'Filter by priority' },
                                    ...
                                    limit: { type: 'number', description: 'Maximum number of tasks to return (default 50)', default: 50 }
                                }
                            }
                        },
                        {
                            name: 'query_projects',
                            description: 'Query projects the user has access to. Use this to get project names, IDs, and details.',
                            input_schema: {
                                type: 'object',
                                properties: {
                                    include_members: { type: 'boolean', description: 'Include project member information', default: false }
                                }
                            }
                        },
                        {
                            name: 'query_team_members',
                            description: 'Query team members (users) in the organization. Use this to find user IDs, names, emails for task assignment.',
                            input_schema: {
                                type: 'object',
                                properties: {}
                            }
                        },
                        {
                            ...
                            input_schema: {
                                ...
                                properties: {
                                    ...
                                    priority: { type: 'string', enum: ['high', 'medium', 'low', 'none'], description: 'Task priority (optional, defaults to none)' },
                                    project_id: { type: 'string', description: 'Project ID (optional, defaults to personal project)' },
                                    assigned_to_id: { type: 'string', description: 'User ID to assign to (optional, defaults to current user)' }
                                },
                                required: ['name']
                            }
                        }
                    ...
                    // Tool execution functions with security
                    const executeTool = async (toolName, toolInput) => {
                        console.log(`[Claude Tools] Executing: ${toolName}`, toolInput);

                        if (toolName === 'query_tasks') {
                            const { filters = {}, limit = 50 } = toolInput;

                            // Build WHERE clause with security constraints
                            let conditions = [];
                            let params = [];

                            // Handle "my tasks" filter (assigned_to_me)
                            if (filters.assigned_to_me) {
                                // Only tasks assigned to the user
                                conditions.push('t.assigned_to_id = ?');
                                params.push(user.id);
                            } else {
                                // All accessible tasks (assigned to user OR in their projects)
                                conditions.push('(t.assigned_to_id = ? OR t.project_id IN (' + projectIds.map(() => '?').join(',') + '))');
                                params.push(user.id, ...projectIds);
                            }
                            ...
                            if (filters.overdue) {
                                conditions.push('t.date < ? AND t.status != ?');
                                params.push(new Date().toISOString().split('T')[0], 'completed');
                            }
                        ...
                        if (toolName === 'query_projects') {
                            const { include_members = false } = toolInput;
                        ...
                        if (toolName === 'query_team_members') {
                        ...
                        if (toolName === 'create_task') {
                            const { name, description, due_date, priority, project_id, assigned_to_id } = toolInput;

                            // Use business logic for proper validation and security
                            const dataService = createDataService(env.DB);
                            const createdTask = await businessLogic.createTask(dataService, user.id, {
                                name,
                                ...
                                assigned_to_id: assigned_to_id || user.id
                            });

                            await logActivity(env.DB, user.id, 'task_created', `Created task "${createdTask.name}" via Claude AI`, createdTask.id, createdTask.project_id);
                            await broadcastChange(env, 'task-created', { taskId: createdTask.id, projectId: createdTask.project_id });

                            return { task: createdTask, message: 'Task created successfully' };
                        }
                    ...
                    while (iterations < maxIterations) {
                        iterations++;
                        console.log(`[Claude Smart] Iteration ${iterations}`);
                        ...
                        const response = await fetch('https://api.anthropic.com/v1/messages', {
                            ...
                            body: JSON.stringify({
                                model: CLAUDE_SONNET,
                                max_tokens: 2048,
                                system: `You are a helpful task management assistant. You have access to tools to query the user's tasks, projects, and team members from their D1 database, and to create tasks.
...
Important context about task queries:
- When the user asks about "my tasks" or "my work", they mean tasks assigned to them (assigned_to_id = user's ID)
- The user has access to tasks in two ways: (1) tasks assigned to them, (2) tasks in projects they are members of
- Always filter for assigned_to_id when user asks about "my" tasks specifically

When answering questions:
1. Use the tools to get up-to-date information from the database
2. Be concise and helpful in your responses
3. Format dates in a human-readable way
4. When listing tasks, include relevant details like status, priority, due date, and project

CRITICAL: Always separate pending tasks from completed tasks in your responses:
- **Pending tasks** are the priority - list these first and in detail
- **Completed tasks** are less important - mention these separately at the end, or just provide a count
- Never mix pending and completed tasks in the same list
- Example format:
  "You have 3 pending tasks:
   ...
   You also have 5 completed tasks."`,
                                messages,
                                tools
                            })
                        });
                        ...
                        if (toolUseBlocks.length === 0) {
                            // No tool use - Claude has given final answer
                            const textBlock = content.find(block => block.type === 'text');
                            const answer = textBlock?.text || 'No response generated';

                            // Check if this was a task creation
                            if (content.some(b => b.type === 'tool_use' && b.name === 'create_task')) {
                                // Look back in conversation for the task creation result
                                // This is a bit hacky but works for Discord response formatting
                                return {
                                    data: {
                                        type: 'answer',
                                        answer: answer
                        ...
                        for (const toolUse of toolUseBlocks) {
                            try {
                                ...
                                console.error(`[Claude Tools] Error executing ${toolUse.name}:`, error);
                                toolResults.push({
                                    type: 'tool_result',
                                    tool_use_id: toolUse.id,
                    ...
                    return {
                        data: {
                            type: 'answer',
                            answer: 'I apologize, but I need more iterations to complete this request. Please try asking in a simpler way.'
                        }
                    };
                }
